...
internal class Butterfly : Tile
{
    ...
    public override void Update(int currentTick, Map map)
    {
            ...
            Y + 1 < map.TileMap.GetLength(1))
        {
            ...
            // todo
                     ...
                     map.TileMap[X - 1, Y].Type == TileType.Space ||
                     map.TileMap[X, Y - 1].Type == TileType.Space ||
                     map.TileMap[X + 1, Y ].Type == TileType.Space)
            {
                map.TileMap[X, Y] = map.TileMap[X, Y];
                map.TileMap[X, Y] = new Space(TileType.Space, Flag.Explodable, 1, X, Y);
                Y++;
                MovedThisTick = true;
            }
    ...
    {
        map.TileMap[X, Y] = new Diamond(TileType.Diamond, Flag.Consumable | Flag.Rounded | Flag.CanFall, 10, X, Y);
        ...
        for (int y = -1; y <= 1; y++)
        {
            for (int x = -1; x <= 1; x++)
            {
                if ((map.TileMap[X + x, Y + y].Flags & Flag.Explodable) != 0)
                    map.TileMap[X + x, Y + y] = new Diamond(TileType.Diamond, Flag.Consumable | Flag.Rounded | Flag.CanFall, 10, X + x, Y + y);
            }
        }
    }
}